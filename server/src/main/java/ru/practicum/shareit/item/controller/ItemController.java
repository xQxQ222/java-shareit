...
public class ItemController {
    ...
    @GetMapping("/search")
    public List<Item> getItemsByCaption(@RequestHeader("X-Sharer-User-Id") Integer userId, @RequestParam(defaultValue = "", required = false) String text) {
    ...
    @PostMapping
    public Item addNewItem(@RequestHeader("X-Sharer-User-Id") Integer userId, @RequestBody ItemRequestDto itemRequestDto) {
        log.info("Пришел POST запрос /items от пользователя с id: {} с телом: {}", userId, itemRequestDto);
        Item item = itemService.addItem(userId, ItemMapper.toDomainModel(null, itemRequestDto, null, null), itemRequestDto.getRequestId() == null ? null : Long.valueOf(itemRequestDto.getRequestId()));
